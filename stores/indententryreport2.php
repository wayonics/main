...
<center> <caption><h1>JYOTIRMAYE TEXTILES PVT. LTD.</caption><br />#11-12-12, RAMIREDDYPET, NARASARAOPET,GUNTUR D.T.</h1></center><br />
 <h2>DEPARTMENT WISE CONSUMPTION STATEMENT ON <?php echo $dt1 ?>  </h2>
...
table
{
 ...
 <th> GENERAL STORES</th>
 <th>MACHINARY SPARES</th>
 <th>OIL& LUBRICANTS</th>
 <th>PACKING MATERIAL</th>
 <th>TOOLS& IMPLEMENT</th>
 ...
 <?php 
 include 'db.php'; 
 $q=mysql_query("SELECT * FROM `indententry` WHERE indentdate  BETWEEN '$dt1' AND '$dt2'")or die(mysql_error());
 
  $q1=mysql_query("SELECT * FROM `indententry` WHERE finhead='$head' OR deptname='$dept' OR machinename='$machine' OR transmode='$transmode'")or die(mysql_error());
 ...
 */
 ...
 { 
 echo "<tr><td><td>$a[total]<td>$a[indentdate]<td>$a[indentslno]<td>$a[itemcode]<td>$a[itemname]<td>$a[indentqty]<td>$a[issuedqty]<td>$a[value]<td>$a[machineno]<td>$a[machinename]<td>$a[deptname]<td>$a[finhead]  ";
 }  
 echo "</tr>";
  /*
